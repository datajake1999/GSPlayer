...
void OutputData(int32 *buf, long count);
...
void OutputData(int32 *buf, int32 count)
{
	...
	if (dpm.encoding & PE_16BIT) {
		...
		if (dpm.encoding & PE_MONO)
			s32tos16_2(buf, (int16*)(Output->lpData + Output->dwBytesRecorded), count);
		else
			s32tos16_2(buf, (int16*)(Output->lpData + Output->dwBytesRecorded), count * 2);
	}
	else {
		/* Convert to 8-bit unsigned. */
		if (dpm.encoding & PE_MONO)
			s32tou8_2(buf, (uint8*)(Output->lpData + Output->dwBytesRecorded), count);
		else
			s32tou8_2(buf, (uint8*)(Output->lpData + Output->dwBytesRecorded), count * 2);
	}

	Output->dwBytesRecorded += converted;
}
